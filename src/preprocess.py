...
def read_pdb_biopython(pdbid, directory, heavy_elements, radii):
    # read PDB protein data using BioPython package
    parser = PDBParser(QUIET=True)  # ignore warnings
    structure = parser.get_structure(pdbid, f"{directory}/{pdbid}/{pdbid}_protein.pdb")
    atoms = []
    # loop over all atoms
